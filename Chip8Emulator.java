...
public class Chip8Emulator extends JPanel implements KeyListener {
    ...
    private int[] display = new int[64 * 32]; // 64x32 pixel display - 1 for on 1 for off
    ...
    public void loadRom(String path) {
        try {
            ...
            for (int i = 0; i < data.length; i++) {
                memory[0x200 + i] = data[i] & 0xFF; //data is already in byte form but just extra checking
            }
        } catch (IOException e) {
    ...
    public void cycle() {
        // Fetch 2-byte opcode
        opcode = (memory[pc] << 8) | memory[pc + 1]; // bytes at the next two program counter locations, combining into 16 bits
        pc += 2;
        ...
        int op = opcode & 0xF000; // for more grouping, we will group based on most significant nibble
        switch (op) {
            case 0x0000:
            ...
            case 0x7000: 
                ...
                break;
            case 0x8000:
                switch (opcode & 0x000F) { // now least significant nibble
                    case 0x0: 
                        op_8XY0();  // Set Vx = Vy
                        break;
                    case 0x1: 
            ...
            case 0xD000: 
                ...
                break; 
            case 0xE000:
                if ((opcode & 0x00FF) == 0x9E) { // Skip next instruction if key with the value of Vx is pressed
                    op_EX9E();
            ...
            case 0xF000:
                switch (opcode & 0x00FF) {
                    ...
                    case 0x55: 
                        ...
                        break; 
                    case 0x65: 
                        op_FX65();  // read registers V0 through Vx from memory starting at I
                        break;
                    default:
                        System.out.printf("Unknown opcode: %X\n", opcode);
                        break;
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int y = 0; y < 32; y++) {
            for (int x = 0; x < 64; x++) { // loop through 64 * 32 display
                int pixel = display[y * 64 + x]; // get pixel value at display
                g.setColor(pixel == 1 ? Color.WHITE : Color.BLACK); // black if 0
                g.fillRect(x * 10, y * 10, 10, 10); // draw pixel at calculated position
            }
        }
    }
    ...
    // keylistener
    @Override
    public void keyPressed(KeyEvent e) { // key event - for when key is pressed
    ...
    public void runEmulator() {
        timer = new Timer(2, new ActionListener() { // will perform an action every 2 milliseconds 
            @Override
